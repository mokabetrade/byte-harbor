...
class GenerationPipeline:
    ...
    def __init__(self, settings: SettingsConf = settings):
        ...
        self.prompting_library = PromptingLibrary.from_file(settings.qwen.prompt_path)
    ...
    async def generate_gs(self, request: GenerateRequest) -> GenerateResponse:
        ...
        background_prompt = self.prompting_library.promptings['background']
        views_prompt = self.prompting_library.promptings['views']
        # Stage 0: Remove background
        images_edited = self.qwen_edit.edit_image(prompt_image=image, seed=request.seed, prompting=background_prompt, encode_prompt=False)
        # Stage 1: Generate novel views
        images_edited += self.qwen_edit.edit_image(prompt_image=images_edited[0], seed=request.seed, prompting=views_prompt)
